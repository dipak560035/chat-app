    ...
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="fixed top-20 right-4 z-50 flex flex-col space-y-2">
        {notifications.map((note, index) => (
          <div
            key={index}
            className="bg-blue-500 text-white px-4 py-2 rounded-lg shadow-lg animate-fade-in-out text-sm"
          >
            {note}
          </div>
        ))}
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, index) => (
          ...
          >
            ...
            >
              {msg.username !== user.username && (
                <p className="text-xs font-bold mb-1 text-blue-500">
                  {msg.username}
                </p>
              )}
